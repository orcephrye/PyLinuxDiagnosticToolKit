...
class SCPClient(object):
    ...
    def put(self, files, remote_path=b'.',
        ...
        if recursive:
            ...
            self._send_files(files, remote_path)
    ...
    def _open(self):
        """open a scp channel"""
        if self.channel is None or not self.transport.is_active():
    ...
    def _read_stats_zip(self, name, zFile):
        return '0644', zFile.getinfo(name).file_size, time.time(), time.time()

    def _read_stats_IO(self, name):
        name.seek(0, os.SEEK_END)
        size = name.tell()
        name.seek(0)
        return '0644', size, time.time(), time.time()

    def _send_files(self, files, remote_path=None):
        for name in files:
            if isinstance(name, IOBase):
                basename = asbytes(os.path.basename(remote_path))
                ...
                with zipfile.ZipFile(sys.argv[0]) as zFile:
                    for tFile in zFile.namelist():
                        if name in tFile:
                            basename = asbytes(os.path.basename(tFile))
                            (mode, size, mtime, atime) = self._read_stats_zip(tFile, zFile)
                            if self.preserve_times:
                                self._send_time(mtime, atime)
                            fl = BytesIO(zFile.open(tFile, 'r').read())
                            self._send_file(fl, basename, mode, size)
                            break
            else:
    ...
    def _recv_all(self):
        ...
        while transport.is_active() and not self.channel.closed:
